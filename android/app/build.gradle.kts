...
val coronaAppPackage = project.findProperty("coronaAppPackage") as? String ?: "com.platopus.cookieCutter"
...
val coronaSrcDir = project.findProperty("coronaSrcDir") as? String
        ?: if (file("$rootDir/../Corona").exists()) {
            "$rootDir/../Corona"
        } else {
            "$rootDir/../some/otherdirectory"
        }
    ...
    val taskCopyResources = tasks.create<Copy>("packageCoronaApp${baseName.capitalize()}") {
        ...
        doFirst {
            if (!file(coronaSrcDir).isDirectory) {
                throw InvalidUserDataException("$coronaSrcDir Unable to find Solar2D project (for example platform/test/assets2/main.lua)!")
            }
        }
    }
...
tasks.register<Copy>("exportToNativeAppTemplate") {
    ...
    from(rootDir) {
        ...
        filter {
            it.replace("com.platopus.cookieCutter", "com.platopus.cookieCutter")
        }
    }
